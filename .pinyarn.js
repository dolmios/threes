...
const config = {
  ...
  yarnUrl:
    'https://raw.githubusercontent.com/yarnpkg/berry/%40yarnpkg/cli/4.0.0-rc.32/packages/yarnpkg-cli/bin/yarn.js',
};

const getUrlHash = (url) =>
  crypto.createHash('sha256').update(url).digest('hex').substring(0, 8);
...
if (config.yarnUrl.includes('/artifacts/')) {
  BERRY_HEADERS['Authorization'] = `token ${config.ghTokens[
    Math.floor(Math.random() * config.ghTokens.length)
  ].join('')}`;
}
...
const PLUGIN_LIST = !fs.existsSync(YARNRC_YML_PATH)
  ? []
  : fs
      .readFileSync(YARNRC_YML_PATH, 'utf-8')
      .split('\n')
      .filter((line) => line.includes('.yarn/plugins/@yarnpkg/plugin-'))
      .map((line) =>
        line.replace(
          /^.*\.yarn\/plugins\/@yarnpkg\/plugin-(.*?)(?:-[0-9a-f]{8})?\.cjs$/,
          '$1'
        )
      );
...
const CURRENT_YARN_URL_HASH = !CURRENT_YARN_BINARYNAME
  ? null
  : path
      .basename(CURRENT_YARN_BINARYNAME)
      .slice(0, -path.extname(CURRENT_YARN_BINARYNAME).length)
      .replace('yarn-', '');
...
if (CURRENT_YARN_URL_HASH !== YARN_URL_HASH) {
  if (CURRENT_YARN_BINARYNAME) {
    if (fs.existsSync(RELEASES_DIR)) fs.rmdirSync(RELEASES_DIR, { recursive: true });
    if (fs.existsSync(PLUGIN_DIR)) fs.rmdirSync(PLUGIN_DIR, { recursive: true });
  }
...
for (const plugin of PLUGIN_LIST) {
  ...
  if (pluginUrl) {
    const pluginPath = path.join(
      PLUGIN_DIR,
      '@yarnpkg',
      `plugin-${plugin}-${getUrlHash(pluginUrl)}.cjs`
    );
...
if (PLUGIN_LIST.length === 0) {
  ...
  for (const entry of entries) {
    ...
    if (
      pluginUrl &&
      (!PLUGIN_LIST.includes(plugin) || getUrlHash(pluginUrl) !== pluginHash)
    )
      fs.unlinkSync(path.join(PLUGIN_DIR, '@yarnpkg', entry));
  }
}

Promise.all(promises).then(() => require(YARN_BINARY), console.error);
